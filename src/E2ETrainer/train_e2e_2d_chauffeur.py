...
if __name__ == '__main__':
    ...
    result_x_p = tf.cast(-result_y * 256.0 / 60.0 + 4 * 256 / 5.0,tf.int32)
    result_y_p = tf.cast(-result_x * 256.0 / 60.0 + 256 / 2.0,tf.int32)
    d_p_x = tf.math.abs(result_y + (tf.cast(result_x_p,tf.float32) - tf.constant(4 * 256 / 5.0))*60.0/256.0 - y_conv[0][0])
    d_p_y = tf.math.abs(result_x + (tf.cast(result_y_p,tf.float32) - tf.constant( 256 / 2.0))*60.0/256.0 - y_conv[0][1])
    ...
    pred_x = -(pix_x - 4.0*256.0/5.0)*60.0/256.0 + y_conv[0][0]
    pred_y = -(pix_y - 128.0)*60.0/256.0 + y_conv[0][1]
    loss_x = tf.math.abs(y_r[0][0] - pred_x)
    loss_y = tf.math.abs(y_r[0][1] - pred_y)
    pred_loss = loss_x + loss_y
    ...
    while(l <= 2000):
        ...
        if((l + 1) % 1 == 0):
            ...
            f = open("loss/model_m0_12_just_ego.txt", "a")

            if(True):
                basermse = rmse
                save_path = saver.save(sess, "model/m0_12_just_ego"+str(l)+".ckpt")
