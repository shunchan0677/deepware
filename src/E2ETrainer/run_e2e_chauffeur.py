...
class RosTensorFlow():
    def __init__(self):
        ...
        self.y_r = tf.placeholder(tf.float32, [None,3])
        ...
        self.y_conv,self.output_bbox_map,self.output_pose_map = AgentNet(self.OGM_pred1, self.image1, self.keep_prob)

        self.pred_m = tf.reshape(self.output_pose_map,(64,64))
        self.pix_x = tf.cast(tf.math.argmax(tf.reduce_max(self.pred_m,reduction_indices=1)),tf.float32)+154.0
        self.pix_y = tf.cast(tf.math.argmax(tf.reduce_max(self.pred_m,reduction_indices=0)),tf.float32)+96.0
        self.pred_x = -(self.pix_x - 4.0*256.0/5.0)*60.0/256.0 + self.y_conv[0][0]
        self.pred_y = -(self.pix_y - 128.0)*60.0/256.0 + self.y_conv[0][1]
        ...
        self.saver.restore(self.sess, "model/m0_12_just_ego12.ckpt")
    ...
    def callback_model(self):
        pred_pose,pred_x1,pred_y1 = self.sess.run([self.y_conv,self.pred_x,self.pred_y], feed_dict={self.OGM_x:self.list_image[:,0:10,:,:,:]*1.0,self.image1: self.list_image[:,10:13,:,:,:]*1.0, self.keep_prob: 0.5})
        #print("model")
        #print(pred_pose)


        self.listener.waitForTransform("/world", "/base_link", rospy.Time.now(), rospy.Duration(4.0))
        (trans, rot) = self.listener.lookupTransformFull("/world", rospy.Time.now(), "/base_link", rospy.Time.now() - rospy.Duration((5)*0.1),"world")
