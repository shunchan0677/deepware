...
class RosTensorFlow():
    def __init__(self):
        ...
        self.y_r = tf.placeholder(tf.float32, [None,12])
        ...
        self.saver.restore(self.sess, "model/m0_9_24.ckpt")
    ...
    def callback_model(self):
        pred_pose = self.sess.run([self.y_conv], feed_dict={self.OGM_x:self.list_image[:,0:10,:,:,:]*1.0,self.image1: self.list_image[:,10:13,:,:,:]*1.0, self.keep_prob: 0.5})
        #print("model")
        #print(pred_pose)
        ...
        pred_x1 = pred_pose[0][0][3*3]
        pred_y1 = pred_pose[0][0][3*3+1]
