...
class GeneratorBuilder:
    ...
    def build(self):
        while True:
            self.random_seq = range(len(self.image_label)-1)
            ...
            for l in range(len(self.image_label)-1):
                ...
                if(flag):
                    ...
                    self.train_r.append(self.r_label[self.random_seq[l],3:15])
...
if __name__ == '__main__':
    ...
    y_r = tf.placeholder(tf.float32, [None,12])
    ...
    reg_loss = tf.losses.mean_squared_error(y_r[0,0],y_conv[0,0])
    reg_loss += tf.losses.mean_squared_error(y_r[0,1],y_conv[0,1])
    reg_loss += tf.losses.mean_squared_error(y_r[0,2],y_conv[0,2])
    reg_loss += tf.losses.mean_squared_error(y_r[0,3],y_conv[0,3])
    reg_loss += tf.losses.mean_squared_error(y_r[0,4],y_conv[0,4])
    reg_loss += tf.losses.mean_squared_error(y_r[0,5],y_conv[0,5])
    reg_loss += tf.losses.mean_squared_error(y_r[0,6],y_conv[0,6])
    reg_loss += tf.losses.mean_squared_error(y_r[0,7],y_conv[0,7])
    reg_loss += tf.losses.mean_squared_error(y_r[0,8],y_conv[0,8])
    reg_loss += tf.losses.mean_squared_error(y_r[0,9],y_conv[0,9])
    reg_loss += tf.losses.mean_squared_error(y_r[0,10],y_conv[0,10])
    reg_loss += tf.losses.mean_squared_error(y_r[0,11],y_conv[0,11])
    ...
    train_step = tf.train.AdamOptimizer(1e-4).minimize(tf.reduce_mean(gen_loss+reg_loss))
    ...
    while(l <= 2000):
        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                loss_tmp = sess.run([train_step,ssim_loss,reg_loss,gen_loss,y_conv], feed_dict={
        ...
        if((l + 1) % 1 == 0):
            ...
            f = open("loss/model_m0_9.txt", "a")

            if(True):
                basermse = rmse
                save_path = saver.save(sess, "model/m0_9_"+str(l)+".ckpt")
