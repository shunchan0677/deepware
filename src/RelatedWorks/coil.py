        ...
        while True:
            ...
            for l in range(len(self.image_label)-1):
                ...
                if(flag):
                    ...
                    way_image = cv2.imread(self.image_label[self.random_seq[l]]+"/waypoint.png",cv2.IMREAD_GRAYSCALE)/255.0
                    if(np.any(way_image[0:100,:]>0.1)):
                        flag =  np.asarray([1,0,0])
                    elif(np.any(way_image[300:400,:]>0.1)):
                        flag =  np.asarray([0,0,1])
                    else:
                        flag =  np.asarray([0,1,0])
                    if((l + 1) % self.batch_size == 0):
                        self.train_image = np.asarray(self.train_image)/ 255.0
                        yield [self.train_image, self.train_r, flag]
                        self.train_image = []
                        self.train_r = []
            yield None
...
if __name__ == '__main__':
    ...
    while(l<=2000):
        ...
        if((l+1)%1 == 0):
            ...
            f = open("loss/coil.txt","a")

            if(valid_rmse<basermse):
                basermse=valid_rmse
                save_path = saver.save(sess, "model/coil.ckpt")
