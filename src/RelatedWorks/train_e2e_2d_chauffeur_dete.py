    ...
    def build(self):
        while True:
            ...
            for l in range(len(self.image_label)-1):
                ...
                anno_image = np.zeros((10,256,256,1))
                ...
                if(flag):
                    for flame in range(self.num_flame):
                        ...
                        anno_image[flame,:,:,:] = np.asarray(cv2.imread(self.image_label[self.random_seq[l]]+"/bbox_"+str(flame)+".png",cv2.IMREAD_GRAYSCALE)).reshape((1,256,256,1))/ 255.0
                    ...
                    self.train_image_anno.append(anno_image)
                    ...
                    if((l + 1) % self.batch_size == 0):
                        ...
                        self.train_image_anno = np.asarray(self.train_image_anno).reshape((-1,self.num_flame,256,256,1))
                        ...
                        yield [self.train_image, self.train_r, self.train_image_anno]
                        self.train_image = []
                        self.train_r = []
                        self.train_image_anno = []
            yield None
...
if __name__ == '__main__':
    print("ChauffeurNet with detection training result log")
        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                loss_tmp = sess.run([train_step,ssim_loss,reg_loss,gen_loss,wp_loss,p_loss,obj_loss,yaw_loss,pred_loss,bbox_loss,result_x_p,result_y_p,pix_x,pix_y,acc_p,image_box_c,p_map,image_np2,input_wp1], feed_dict={
                         OGM_x:train_image_result[:,0:10,:,:,:],OGM_y:train_image_anno[:,0:10,:,:,:],image1: train_image[:,10:13,:,:,:], y_r: train_r, keep_prob: 0.5})
                loss_a += loss_tmp[1]
                loss_b += loss_tmp[2]
                loss_c += loss_tmp[3]
                loss_d += loss_tmp[4]
        ...
        if((l + 1) % 1 == 0):
            ...
            for i, valid_data in enumerate(valid_new_generator):
                if not valid_data:
                    ...
                    result = sess.run([pred_loss,y_conv], feed_dict={OGM_x:valid_image_result[:,0:10,:,:,:],OGM_y:valid_image_anno[:,0:10,:,:,:],image1: valid_image[:,10:13,:,:,:], y_r: valid_r, keep_prob: 1.0})
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    result = sess.run([pred_loss,y_conv], feed_dict={OGM_x:test_image_result[:,0:10,:,:,:],OGM_y:test_image_anno[:,0:10,:,:,:],image1: test_image[:,10:13,:,:,:], y_r: test_r, keep_prob: 1.0})
                    rmse = rmse + result[0]
                    #print(result[1])
            ...
            f = open("loss/model_ours_dete.txt", "a")

            if(trainrmse > valid_rmse):
                trainrmse = valid_rmse
                save_path = saver.save(sess, "model/ours"+str(l)+"_dete.ckpt")
