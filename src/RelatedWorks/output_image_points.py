            ...
            for l in range(len(self.image_label)-1):
                ...
                if(flag):
                    ...
                    for flame in range(self.num_flame):
                        ...
                        tf_y = copy.copy(self.train_r[flame][1])
                        image[flame,:,:,:],self.train_r[flame][0],self.train_r[flame][1] = image_rotation(self.image_label[self.random_seq[l]]+"/obj_bbox_"+str(flame)+"_.jpg",tf_x,tf_y,rand_int)
...
if __name__ == '__main__':
    ...
    ego_loss = gen_loss + yaw_loss + p_loss + simple_loss  + bbox_loss 
    ...
    saver.restore(sess, "model/ae_sim_carla_true53.ckpt")
...
OGM_x:valid_image_result[:,0:10,:,:,:],OGM_y:valid_image[:,0:10,:,:,:],ego_bbox_x:valid_image_result[:,10:20,:,:,:],ego_bbox_y:valid_image[:,10:20,:,:,:],ego_p_x:valid_image_result[:,20:30,:,:,:],ego_p_y:valid_image[:,20:30,:,:,:],maps_x:valid_image[:,30:,:,:,:], y_r: valid_r, keep_prob: 1.0})
            ...
            video = cv2.VideoWriter('video_1.mp4', fourcc, 20.0, (400, 400))
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    result = sess.run([pred_loss,OGM_pred1,y_conv], feed_dict={OGM_x:test_image_result[:,0:10,:,:,:],OGM_y:test_image[:,0:10,:,:,:],ego_bbox_x:test_image_result[:,10:20,:,:,:],ego_bbox_y:test_image[:,10:20,:,:,:],ego_p_x:test_image_result[:,20:30,:,:,:],ego_p_y:test_image[:,20:30,:,:,:],maps_x:test_image[:,30:,:,:,:], y_r: test_r, keep_prob: 1.0})
                    rmse = rmse + result[0]
                    #print(result[1])
                    map1 = test_image[:,30,:,:,:]
                    map1 = map1.reshape((1,400,400))
                    image_map = np.zeros((400,400,3))
                    image_map[:,:,0]=map1[0]
                    image_map[:,:,1]=map1[0]
                    image_map[:,:,2]=map1[0]
                    wp  = test_image[:,31,:,:,:]
                    wp= wp.reshape((1,400,400))
                    image_wp = np.zeros((400,400,3))
                    image_wp[:,:,0]=wp[0]
                    ogms = result[1]
                    image = np.zeros((1,400,400))
                    image_c = np.zeros((400,400,3))
                    for q in range(10):
                        ogm = ogms[q,:,:,0]
                        color = 255*(q%5+1)/5
                        image = np.where(ogm>=0.5,color,image)
                        if(q == 4):
                            image_c[:,:,2] = image
                            image = np.zeros((1,400,400))
                        if(q == 9):
                            image_c[:,:,1] = image
                    image_c = np.where(image_map>0.5,255,image_c)
                    image_wp[:,:,0] = np.where(image_c[:,:,1] < 10,image_wp[:,:,0],0)
                    image_wp[:,:,0] = np.where(image_c[:,:,2] < 10,image_wp[:,:,0],0)
                    image_c = np.where(image_c < 10,image_wp*128,image_c)
                    cv2.imwrite("index_1"+str(q)+".png",image_c)
                    image_c = cv2.imread("index_1"+str(q)+".png")
            ...
            f = open("loss/model_ae_sim_ours_carla.txt", "a")

            if(trainrmse > valid_rmse):
                trainrmse = valid_rmse
                save_path = saver.save(sess, "model/ae_sim_ours_carla"+str(l)+".ckpt")
