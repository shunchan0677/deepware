        ...
        while True:
            ...
            for l in range(300,len(self.image_label)-1):
                ...
                if(flag):
                    ...
                    if(self.train_r[-1][0]>0.0):
                        rand_int_01 = random.randint(0,4)
                        if(rand_int_01 == 0):
                    ...
                    for flame in range(self.num_flame):
                        ...
                        image[flame,:,:,:],self.train_r[flame][0],self.train_r[flame][1] = image_rotation(self.image_label[self.random_seq[l]]+"/occupancy_grid_"+str(flame)+"_.jpg",tf_x,tf_y,rand_int)
                        image[flame+10,:,:,:],a,b = image_rotation(self.image_label[self.random_seq[l]]+"/ego_bbox_"+str(flame)+"_.jpg",tf_x,tf_y,rand_int)
                    ...
                    image[30,:,:,:],a,b = image_rotation(self.image_label[self.random_seq[l]]+"/map_image_.jpg",tf_x,tf_y,rand_int)
                    if(tl[4]=="False"):
                        image[31,:,:,:],a,b = image_rotation(self.image_label[self.random_seq[l]]+"/way_image_.jpg",tf_x,tf_y,rand_int)
...
if __name__ == '__main__':
    ...
    simple_loss = tf.math.sqrt(tf.math.abs(y_r[:,0] - y_conv[:,0])**2 + 100*tf.math.abs(y_r[:,1] - y_conv[:,1])**2)
    pred_loss = simple_loss[9]



    ego_loss = gen_loss + simple_loss + 100*wp_loss
        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                loss_tmp = sess.run([train_step,ssim_loss,reg_loss,gen_loss,wp_loss,p_loss,obj_loss,yaw_loss,pred_loss,bbox_loss], feed_dict={
                         OGM_x:train_image_result[:,0:10,:,:,:],OGM_y:train_image[:,0:10,:,:,:],ego_bbox_x:train_image_result[:,10:20,:,:,:],ego_bbox_y:train_image[:,10:20,:,:,:],ego_p_x:train_image_result[:,20:30,:,:,:],ego_p_y:train_image[:,20:30,:,:,:],maps_x:train_image[:,30:,:,:,:]*10.0, y_r: train_r, keep_prob: 0.5})
                loss_a += loss_tmp[1]
                loss_b += loss_tmp[2]
                loss_c += loss_tmp[3]
                loss_d += loss_tmp[4]
...
OGM_x:valid_image_result[:,0:10,:,:,:],OGM_y:valid_image[:,0:10,:,:,:],ego_bbox_x:valid_image_result[:,10:20,:,:,:],ego_bbox_y:valid_image[:,10:20,:,:,:],ego_p_x:valid_image_result[:,20:30,:,:,:],ego_p_y:valid_image[:,20:30,:,:,:],maps_x:valid_image[:,30:,:,:,:]*10.0, y_r: valid_r, keep_prob: 1.0})
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    result = sess.run([pred_loss,y_conv], feed_dict={OGM_x:test_image_result[:,0:10,:,:,:],OGM_y:test_image[:,0:10,:,:,:],ego_bbox_x:test_image_result[:,10:20,:,:,:],ego_bbox_y:test_image[:,10:20,:,:,:],ego_p_x:test_image_result[:,20:30,:,:,:],ego_p_y:test_image[:,20:30,:,:,:],maps_x:test_image[:,30:,:,:,:]*10.0, y_r: test_r, keep_prob: 1.0})
                    rmse = rmse + result[0]
                    #print(result[1])
            ...
            f = open("loss/20200103model_ae_sim_ours_carla_true_y_10xmap_wp_0_2aug.txt", "a")

            if(trainrmse > valid_rmse):
                trainrmse = valid_rmse
                save_path = saver.save(sess, "model/20200103ae_sim_ours_carla_true_y_10xmap_wp_0_2aug"+str(l)+".ckpt")
