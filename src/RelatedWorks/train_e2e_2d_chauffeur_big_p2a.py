    ...
    def build(self):
        while True:
            ...
            for l in range(len(self.image_label)-1):
                ...
                if(os.path.exists(path) == False):
                    flag = False
                anno_image = np.zeros((10,400,400,1))
                if(flag):
                    for flame in range(self.num_flame):
                        image[flame,:,:,:] = np.asarray(cv2.imread(self.image_label[self.random_seq[l]]+"/lidar_"+str(flame)+".png",cv2.IMREAD_GRAYSCALE)).reshape((1,400,400,1))/ 255.0
                        anno_image[flame,:,:,:] = np.asarray(cv2.imread(self.image_label[self.random_seq[l]]+"/bbox_"+str(flame)+".png",cv2.IMREAD_GRAYSCALE)).reshape((1,400,400,1))/ 255.0
                    image[10,:,:,:] = np.asarray(cv2.imread(self.image_label[self.random_seq[l]]+"/ego_pose.png",cv2.IMREAD_GRAYSCALE)).reshape((1,400,400,1))/ 255.0
                    image[11,:,:,:] = np.asarray(cv2.imread(self.image_label[self.random_seq[l]]+"/map.png",cv2.IMREAD_GRAYSCALE)).reshape((1,400,400,1))/ 255.0
                    image[12,:,:,:] = np.asarray(cv2.imread(self.image_label[self.random_seq[l]]+"/waypoint.png",cv2.IMREAD_GRAYSCALE)).reshape((1,400,400,1))/ 255.0
                    self.train_image.append(image)
                    self.train_r.append(self.r_label[self.random_seq[l]][12:15])
                    if((l + 1) % self.batch_size == 0):
                        self.train_image = np.asarray(self.train_image).reshape((-1,self.num_flame+3,400,400,1))
                        anno_image = np.asarray(anno_image).reshape((-1,10,400,400,1))
                        ...
                        yield [self.train_image, self.train_r, anno_image]
                        self.train_image = []
                        self.train_r = []
            yield None
...
def make_datalist(filenames):
    ...
    for filename in filenames:
        image_file = glob.glob("/home/s_seiya/tmp/lyft_data_big/" +
                               filename + "/*")
        image_file.sort()
        train_image_label+=image_file

        r = np.loadtxt("/home/s_seiya/tmp/lyft_data_big/" + filename +
...
def AgentNet(ogm, image, keep_prob):
    ogm1 = tf.reshape(ogm,[-1,10,400,400,1])
    inputs = tf.concat([ogm1,image],1)
    inputs = tf.reshape(inputs,[1]+[400,400]+[13])

    with tf.name_scope('conv1') as scope:
        W_conv1 = weight_variable([5, 5, 13, 24])
    ...
    h_conv6_flatten = tf.reshape(h_pool6, [1, 7 * 7 * 64])
    ...
    h_fc1_d_flatten = tf.reshape(h_pool2, [ 1, 100, 100 ,36])


    with tf.name_scope('de-conv4') as scope:
        W_conv4_d = weight_variable([5, 5, 36, 2])
        b_conv4_d = bias_variable([2])
        h_conv4_d = conv2d(h_fc1_d_flatten, W_conv4_d) + b_conv4_d


    h_conv5_d1 = tf.reshape(h_conv4_d, [ 2, 100, 100 ,1])

    p_map = h_conv5_d1[0]

    p_map_soft = tf.nn.softmax(tf.reshape(p_map,[-1,100*100]))
    p_out = tf.reshape(p_map_soft,[100,100,1])
...
if __name__ == '__main__':
    ...
    OGM_x = tf.placeholder(tf.float32, [None,10] + [400, 400]+ [1])
    image1 = tf.placeholder(tf.float32, [None,3] + [400, 400]+ [1])
    OGM_y1 = tf.placeholder(tf.float32, [None,10] + [400, 400]+ [1])
    ...
    y_conv,output_bbox_map,output_pose_map = AgentNet(OGM_pred1, image1, keep_prob)
    ...
    image_np0 = tf.image.resize_images(image_np,(400/down_factor,400/down_factor))

    num_pix = tf.reduce_sum(image_np0)

    image_np2 = tf.reshape(image_np0,(400/down_factor,400/down_factor,1))
    image_np2_c = image_np2#tf.image.crop_to_bounding_box(image_np2,240,150,100,100)
    bbox_loss = -tf.reduce_mean(tf.log(tf.clip_by_value(output_bbox_map,1e-20,1e+20))*image_np2_c + tf.log(tf.clip_by_value(1.-output_bbox_map,1e-20,1e+20))*(tf.ones([100,100]) - image_np2_c) )
    ...
    d_p_x = tf.math.abs(result_y + (tf.cast(result_x_p,tf.float32) - tf.constant(4 * (400 /down_factor) / 5.0))*80.0/(400.0/down_factor) - y_conv[0][0])
    d_p_y = tf.math.abs(result_x + (tf.cast(result_y_p,tf.float32) - tf.constant( (400/down_factor) / 2.0))*80.0/(400.0/down_factor) - y_conv[0][1])
    cond_p = tf.math.logical_and(tf.equal(row_idx1,result_y_p),tf.equal(col_idx1,result_x_p))
    p_map = tf.where(cond_p,tf.ones([(400/down_factor),(400/down_factor)]), tf.zeros([(400/down_factor),(400/down_factor)]))
    p_map_g = tf.exp(-(tf.cast(row_idx1-result_y_p,tf.float32)**2 + tf.cast(col_idx1-result_x_p,tf.float32)**2)/tf.constant(2.0*0.5))

    p_map = tf.reshape(p_map,((400/down_factor),(400/down_factor),1))
    p_map_g = tf.reshape(p_map_g,((400/down_factor),(400/down_factor),1))
    p_map_c = p_map#tf.image.crop_to_bounding_box(p_map,240,150,100,100)
    p_map_g_c = p_map_g#tf.image.crop_to_bounding_box(p_map_g,240,150,100,100)
    ...
    pred_m = tf.reshape(p_map,(400/down_factor,400/down_factor))
    ...
    pred_x = -(pix_x - 4.0*(400/down_factor)/5.0)*80.0/(400/down_factor) + y_conv[0][0]
    pred_y = -(pix_y - (400/down_factor)/2.0)*80.0/(400/down_factor) + y_conv[0][1]
    loss_x = tf.math.abs(y_r[0][0] - pred_x)
    loss_y = tf.math.abs(y_r[0][1] - pred_y)
    pred_loss = tf.math.sqrt(loss_x**2 + loss_y**2)

    simple_loss = tf.math.sqrt(tf.math.abs(y_r[0][0] - y_conv[0][0])**2 + tf.math.abs(y_r[0][1] - y_conv[0][1])**2)
    #print p_map
    #print output_pose_map
    #print pred_los
    pred_loss = simple_loss

    ego_loss =  simple_loss + gen_loss + yaw_loss #p_loss + reg_loss + yaw_loss  +bbox_loss
    ...
    while(l <= 2000):
        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                loss_tmp = sess.run([train_step,ssim_loss,reg_loss,gen_loss,wp_loss,p_loss,obj_loss,yaw_loss,pred_loss,bbox_loss,result_x_p,result_y_p,pix_x,pix_y,acc_p,image_box_c,p_map,image_np2,input_wp1], feed_dict={
                         OGM_x:train_image_result[:,0:10,:,:,:],OGM_y:train_anno[:,0:10,:,:,:],image1: train_image[:,10:13,:,:,:], y_r: train_r, keep_prob: 0.5})
                loss_a += loss_tmp[1]
                loss_b += loss_tmp[2]
                loss_c += loss_tmp[3]
                loss_d += loss_tmp[4]
        ...
        if((l + 1) % 1 == 0):
            ...
            for i, valid_data in enumerate(valid_new_generator):
                if not valid_data:
                    ...
                    result = sess.run([pred_loss,y_conv], feed_dict={OGM_x:valid_image_result[:,0:10,:,:,:],OGM_y:valid_anno[:,0:10,:,:,:],image1: valid_image[:,10:13,:,:,:], y_r: valid_r, keep_prob: 1.0})
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    result = sess.run([pred_loss,y_conv], feed_dict={OGM_x:test_image_result[:,0:10,:,:,:],OGM_y:test_anno[:,0:10,:,:,:],image1: test_image[:,10:13,:,:,:], y_r: test_r, keep_prob: 1.0})
                    rmse = rmse + result[0]
                    #print(result[1])
            ...
            f = open("loss/model_ours_im_big_conv.txt", "a")

            if(trainrmse > valid_rmse):
                trainrmse = valid_rmse
                save_path = saver.save(sess, "model/ours"+str(l)+"_im_big_conv.ckpt")
