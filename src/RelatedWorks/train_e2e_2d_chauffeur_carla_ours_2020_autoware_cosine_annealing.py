        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                loss_tmp = sess.run([train_step,ssim_loss,reg_loss,gen_loss,wp_loss,p_loss,obj_loss,yaw_loss,pred_loss,bbox_loss], feed_dict={
                         OGM_x:train_image_result[:,0:10,:,:,:],OGM_y:train_image[:,0:10,:,:,:],ego_bbox_x:train_image_result[:,10:20,:,:,:],ego_bbox_y:train_image[:,10:20,:,:,:],ego_p_x:train_image_result[:,20:30,:,:,:],ego_p_y:train_image[:,20:30,:,:,:],maps_x:train_image[:,30:,:,:,:], y_r: train_r, keep_prob: 0.5, global_step: global_step_now,decay_steps: float(len(train_image_list))})
                loss_a += loss_tmp[1]
                loss_b += loss_tmp[2]
                loss_c += loss_tmp[3]
                loss_d += loss_tmp[4]
...
OGM_x:valid_image_result[:,0:10,:,:,:],OGM_y:valid_image[:,0:10,:,:,:],ego_bbox_x:valid_image_result[:,10:20,:,:,:],ego_bbox_y:valid_image[:,10:20,:,:,:],ego_p_x:valid_image_result[:,20:30,:,:,:],ego_p_y:valid_image[:,20:30,:,:,:],maps_x:valid_image[:,30:,:,:,:], y_r: valid_r, keep_prob: 1.0})
            ...
            f = open("loss/20200804_autoware19_data_oversample8_no_hdmap_cosine.txt", "a")

            if(trainrmse > valid_rmse):
                trainrmse = valid_rmse
                save_path = saver.save(sess, "model/2020804_autoware19_data_oversample8_no_hdmap_cosine"+str(l)+".ckpt")
