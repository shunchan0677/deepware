        ...
        while True:
            ...
            for l in range(len(self.image_label)-1):
                ...
                for flame in range(self.num_flame):
                    path = self.image_label[self.random_seq[l]]+"/lidar_"+str(flame)+".png"
...
def AgentNet(ogm, ego_bbox, ego_p, maps, keep_prob):
    ...
    #p_map = tf.reshape(p_map,[400,400,10])
    #print(tf.reduce_sum(tf.reduce_sum(tf.exp(p_map), 2),2))
    p_map_soft = tf.exp(p_map) / tf.reduce_sum(tf.reduce_sum(tf.exp(p_map), 0),0)
    p_out = tf.reshape(p_map_soft,[1,10,400,400,1])

    return tf.reshape(y_conv,[10,3]), tf.sigmoid(h_conv5_d1[0]) ,p_out#tf.reshape(tf.sigmoid(h_conv5_d1[1]),[1,10,400,400,1])
...
if __name__ == '__main__':
    ...
    result_x = y_r[:,0]#result[0]
    result_y = y_r[:,1]#result[1]
    ...
    #gauss_map = tf.zeros([10,400,400,1])
    gauss_map = []

    row_idx1,col_idx1 = tf.meshgrid(tf.range(400/down_factor),tf.range(400/down_factor))
    
    result_x_p = tf.cast(result_x * (400.0 ) / 80.0 ,tf.int32)
    result_y_p = tf.cast(result_y * (400.0 ) / 80.0 ,tf.int32)

    d_p_x = tf.math.abs(result_x - tf.cast(result_x_p,tf.float32)*0.2 - y_conv[:,0])
    d_p_y = tf.math.abs(result_y - tf.cast(result_y_p,tf.float32)*0.2 - y_conv[:,1])

    #for i in range(10):
    #    gauss_map.append(tf.exp(-(tf.cast(row_idx1-result_y_p[i],tf.float32)**2 + tf.cast(col_idx1-result_x_p[i],tf.float32)**2)/tf.constant(2.0*3.0)))
    #gauss_map = tf.convert_to_tensor(gauss_map, dtype=tf.float32)
    #gauss_map = tf.reshape(gauss_map,[1,10,400,400,1])
    ...
    p_loss = -tf.reduce_sum(tf.log(tf.clip_by_value(output_pose_map,1e-20,1e+20))*p_map_c)#*(1.-output_pose_map) + tf.log(tf.clip_by_value(1.-output_pose_map,1e-20,1e+20))*(1.-p_map_c)*(1.-gauss_map)*output_pose_map)
    ...
    pred_m = ego_p_y[0,:,:,:,0]#tf.reshape(ego_p_y,(400/down_factor,400/down_factor,10))

    pix_x = tf.cast(tf.math.argmax(tf.reduce_max(pred_m,reduction_indices=2),1),tf.float32)
    pix_y = tf.cast(tf.math.argmax(tf.reduce_max(pred_m,reduction_indices=1),1),tf.float32)
    pred_x = -(pix_x - 4.0*(400/down_factor)/5.0)*80.0/(400.0/down_factor) + (result_x - tf.cast(result_x_p,tf.float32)*0.2)#y_conv[:,0]
    pred_y = -(pix_y - (400/down_factor)/2.0)*80.0/(400.0/down_factor)# - (result_y - tf.cast(result_y_p,tf.float32)*0.2)#y_conv[:,1]
    loss_x = tf.math.abs(y_r[:,0] - pred_x)
    loss_y = tf.math.abs(y_r[:,1] - pred_y)
    pred_loss_all = tf.math.sqrt(loss_x**2 + loss_y**2)
    pred_loss = loss_x[9]#pred_loss_all[9]
    #pred_loss_all = tf.where(pred_loss_all<1, 0.5*pred_loss_all**2, pred_loss_all-0.5 ) #smooth l1 loss


    #simple_loss = tf.math.sqrt(tf.math.abs(y_r[:,0] - y_conv[:,0])**2 + tf.math.abs(y_r[:,1] - y_conv[:,1])**2)
    #pred_loss = simple_loss

    ego_loss = gen_loss + yaw_loss + p_loss + reg_loss  + bbox_loss
...
OGM_x:valid_image_result[:,0:10,:,:,:],OGM_y:valid_image[:,0:10,:,:,:],ego_bbox_x:valid_image_result[:,10:20,:,:,:],ego_bbox_y:valid_image[:,10:20,:,:,:],ego_p_x:valid_image_result[:,20:30,:,:,:],ego_p_y:valid_image[:,20:30,:,:,:],maps_x:valid_image[:,30:,:,:,:], y_r: valid_r, keep_prob: 1.0})
            ...
            f = open("loss/model_ae_ours.txt", "a")

            if(trainrmse > valid_rmse):
                trainrmse = valid_rmse
                save_path = saver.save(sess, "model/ae_ours"+str(l)+".ckpt")
