    ...
    def build(self):
        while True:
            ...
            for l in range(300,len(self.image_label)-1):
                ...
                for flame in range(self.num_flame):
                    if(flame <= 4):
                        twist_list.append([self.train_r[flame][0],self.train_r[flame][1],self.train_r[flame][2],0.0])
                    else:
                        x = self.train_r[-1][0]
                        y = self.train_r[-1][1]
                        if(y!=0):
                            ...
                            if(r**2 - x**2 > 0):
                                if(y > 0):
                                    ...
                                    y = r + np.sqrt(r**2 - x**2)
                            twist_list.append([x,y,0.0,0.0])
                        else:
                            x = self.train_r[flame][0]
                            twist_list.append([x,0.0,0.0,0.0])
...
def AgentNet(ogm, ego_bbox, ego_p, maps, keep_prob):
    ...
    for i in range(10):
        inputs = tf.concat([[tf.where(maps[1,:,:,:]>0.1,ogm[i,:,:,:],tf.zeros([400,400,1]))],[ego_bbox[i,:,:,:]]],0)
    ...
    return tf.reshape(y_conv,[10,4]), tf.sigmoid(h_conv5_d1[1]) ,p_out
...
if __name__ == '__main__':
    ...
    y_r = tf.placeholder(tf.float32, [10,4])
    ...
    map_loss = input_map[p_9_x,p_9_y]#tf.reduce_mean(tf.cast(output_bbox_map,tf.float32) * input_map_c)
    obj_loss = input_obj[p_9_x,p_9_y]#tf.reduce_mean(tf.cast(output_bbox_map,tf.float32)*input_obj_c)
    ...
    simple_loss = tf.math.sqrt(tf.math.abs(y_r[:,0] - y_conv[:,0])**2 + 2.0*tf.math.abs(y_r[:,1] - y_conv[:,1])**2)
    y_soft_max = [tf.nn.softmax(y_conv[9,2:4])]
    #print(y_soft_max.shape)
    ans = [y_r[9,2:4]]
    weight = tf.constant([0.7,0.3])
    class_loss = -tf.reduce_sum(ans*weight*tf.log(y_soft_max)) #weighted
    pred_loss = simple_loss[9]
    correct_prediction = tf.equal(tf.argmax(y_soft_max,1), tf.argmax(ans,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"))
    pred_res = tf.reduce_mean(tf.cast(tf.argmax(y_soft_max,1), "float"))
    ans_res = tf.reduce_mean(tf.cast(tf.argmax(ans,1), "float"))


    ego_loss = gen_loss + simple_loss*100.0 + class_loss*100.0
    env_loss = 100.0*wp_loss*np.sqrt(2)
    ...
    learning_loss = tf.train.cosine_decay(1e-5, global_step, decay_steps, alpha=0.0, name=None)

    train_step = tf.train.AdamOptimizer(learning_loss).minimize(total_loss)
    ...
    filenames = ["_2020-06-14-12-09-28","_2020-06-14-12-27-11","_2020-06-14-12-46-57","_2020-06-14-13-03-39","_2020-06-14-13-13-43","_2020-06-15-15-06-06","_2020-06-15-15-20-37","_2020-06-15-15-29-50","_2020-06-15-16-11-13","_2020-06-16-00-44-36","_2020-06-16-01-02-06","_2020-06-16-01-12-50","_2020-06-16-01-31-15","_2020-06-16-02-27-39","_2020-06-16-02-41-39","_2020-06-16-02-50-23"]
    train_image_list,train_r_label,train_tl_label = make_datalist(filenames)

    filenames = ["_2020-06-14-11-18-04","_2020-06-14-11-35-41","_2020-06-16-03-07-25","_2020-06-16-03-27-07","_2020-06-16-06-12-59","_2020-06-16-06-33-43","_2020-06-16-06-51-55","_2020-06-16-07-11-28"]
    ...
    while(l <= 2000):
        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                train_image_result = copy.deepcopy(train_image)
                ...
                train_image_result[:,30,:,:,:] = np.zeros((1,400,400,1))



                loss_tmp = sess.run([train_step,ssim_loss,accuracy,gen_loss,pred_res,ans_res,obj_loss,yaw_loss,pred_loss,bbox_loss], feed_dict={
                         OGM_x:train_image_result[:,0:10,:,:,:],OGM_y:train_image[:,0:10,:,:,:],ego_bbox_x:train_image_result[:,10:20,:,:,:],ego_bbox_y:train_image[:,10:20,:,:,:],ego_p_x:train_image_result[:,20:30,:,:,:],ego_p_y:train_image[:,20:30,:,:,:],maps_x:train_image_result[:,30:,:,:,:], y_r: train_r, keep_prob: 0.5, global_step: global_step_now,decay_steps: float(len(train_image_list))})
                loss_a += loss_tmp[1]
                loss_b += loss_tmp[2]
                loss_c += loss_tmp[3]
                loss_d += loss_tmp[4]
                ...
                global_step_now += 1.0
                if(i % 100 == 0 and i != 0):
                    print("train_step: " + str(i) +", ssim: "+str(loss_a/1)+", acc_loss: "+str(loss_b/100)+", gen_loss: "+str(loss_c/100)+", pred_res: "+str(loss_d/100)+", ans_res: "+str(loss_e/100)+", obj_loss: "+str(loss_f/100)+", yaw_loss: "+str(loss_g/100)+", pred_loss: "+str(loss_h/100)+", bbox_loss: "+str(loss_j/100))
        ...
        if((l + 1) % 1 == 0):
            ...
            for i, valid_data in enumerate(valid_new_generator):
                if not valid_data:
                    ...
                    valid_image_result = copy.deepcopy(valid_image)
                    ...
                    valid_image_result[:,30,:,:,:] = np.zeros((1,400,400,1))
                    result = sess.run([pred_loss,y_conv], feed_dict={
OGM_x:valid_image_result[:,0:10,:,:,:],OGM_y:valid_image[:,0:10,:,:,:],ego_bbox_x:valid_image_result[:,10:20,:,:,:],ego_bbox_y:valid_image[:,10:20,:,:,:],ego_p_x:valid_image_result[:,20:30,:,:,:],ego_p_y:valid_image[:,20:30,:,:,:],maps_x:valid_image_result[:,30:,:,:,:], y_r: valid_r, keep_prob: 1.0})
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    test_r = test_data[1]
                    test_image_result = copy.deepcopy(test_image)
                    ...
                    test_image_result[:,30,:,:,:] = np.zeros((1,400,400,1))

                    result = sess.run([pred_loss,y_conv], feed_dict={OGM_x:test_image_result[:,0:10,:,:,:],OGM_y:test_image[:,0:10,:,:,:],ego_bbox_x:test_image_result[:,10:20,:,:,:],ego_bbox_y:test_image[:,10:20,:,:,:],ego_p_x:test_image_result[:,20:30,:,:,:],ego_p_y:test_image[:,20:30,:,:,:],maps_x:test_image_result[:,30:,:,:,:], y_r: test_r, keep_prob: 1.0})
                    rmse = rmse + result[0]
                    #print(result[1])
            ...
            f = open("loss/20200804_autoware19_data_oversample8_forstop_class100_no_map_way_conv01_weighted_cosine.txt", "a")

            if(trainrmse > valid_rmse):
                trainrmse = valid_rmse
                save_path = saver.save(sess, "model/2020804_autoware19_data_oversample8_forstop_class100_nomap_way_conv01_weighted_cosine_"+str(l)+".ckpt")
