            ...
            for l in range(len(self.image_label)-1):
                ...
                if(flag):
                    ...
                    for i in range(10):
                        ...
                        #self.train_r.append(x)
                        #self.train_r.append(y)
                        #self.train_r.append(yaw)
...
def AgentNet(ogm, ego_bbox, ego_p, maps, keep_prob):
    ...
    with tf.name_scope('age-dec-conv3') as scope:
        W_conv6 = weight_variable([3, 3, 3, 8])
        b_conv6 = bias_variable([3])
        h_conv6 = deconv2d(h_conv5, W_conv6, 400,3,10) + b_conv6
    ...
    h_conv5_d1 = tf.reshape(h_conv6, [ 3, 400, 400 ,10])
    ...
    return tf.reshape(y_conv,[10,3]), tf.sigmoid(h_conv5_d1[1]) ,p_out, tf.reshape(tf.sigmoid(h_conv5_d1[2]),[1,10,400,400,1])
...
if __name__ == '__main__':
    ...
    y_conv,output_bbox_map,output_pose_map,OGM_pred = AgentNet(OGM_x, ego_bbox_x, ego_p_x, maps_x, keep_prob)
    OGM_pred1 = tf.where(OGM_pred > 0.1,tf.ones([1,10,400,400,1]),tf.zeros([1,10,400,400,1]))
...
OGM_x:valid_image_result[:,0:10,:,:,:],OGM_y:valid_image[:,0:10,:,:,:],ego_bbox_x:valid_image_result[:,10:20,:,:,:],ego_bbox_y:valid_image[:,10:20,:,:,:],ego_p_x:valid_image_result[:,20:30,:,:,:],ego_p_y:valid_image[:,20:30,:,:,:],maps_x:valid_image[:,30:,:,:,:], y_r: valid_r, keep_prob: 1.0})
            ...
            f = open("loss/model_ae_sim_ours2_carla_true.txt", "a")

            if(trainrmse > valid_rmse):
                trainrmse = valid_rmse
                save_path = saver.save(sess, "model/ae_sim_ours2_carla_true"+str(l)+".ckpt")
