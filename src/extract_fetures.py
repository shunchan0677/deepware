...
class FeatureExtractor():
    def __init__(self,flag):
        ...
        self.tlr = TrafficLight()
        self.tlr.traffic_light = 2
        self.tlr_count = 0
        ...
        self.sub_vec = rospy.Subscriber(
            'vector_map', MarkerArray, self.callback_vec)
        self.sub_pc = rospy.Subscriber(
            'filtered_points', PointCloud2, self.callback_pc)
        self.sub_obj = rospy.Subscriber(
            'detection/lidar_detector/objects', DetectedObjectArray, self.callback_obj)
        self.sub_tlr = rospy.Subscriber(
            'light_color', TrafficLight, self.callback_tlr)
    ...
    def update_map_points(self):
        ...
        for mark in self.marker.markers:
            if(mark.ns == "white_line" or mark.ns == "stop_line" or mark.ns == "cross_walk" or mark.ns == "road_edge"):
    ...
    def update_image_waypoint(self, image_np):
        ...
        if (self.tlr.traffic_light == 0):
            self.color = (0, 0, 255)
            self.tlr_count = 1
        elif (self.tlr.traffic_light == 1):
            self.color = (255, 0, 0)
            self.tlr_count = 1
        elif(self.tlr.traffic_light == 2):
            self.tlr_count += 1
    ...
    def update_image_ego(self, image_np):
        ...
        for i in range(len(self.old_tf)-5):
    ...
    def callback_pc(self, points_clound):
        ...
        (self.trans, self.rot) = self.listener.lookupTransform(self.world_frame, self.ego_frame, rospy.Time(0))
    ...
    def update_tf(self):
        for i in range(10):
            ...
            (trans, rot) = self.listener.lookupTransformFull("/world", rospy.Time.now(), "/base_link", rospy.Time.now() - rospy.Duration((12-i)*0.1),self.world_frame)
