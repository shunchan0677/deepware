        ...
        while True:
            ...
            for l in range(len(self.image_label)):
                ...
                if((l + 1) % self.batch_size == 0):
                    ...
                    self.train_image1 = np.zeros((self.num_flame,256,256,2))
                    self.train_image1[:,:,:,0] = self.train_image[:,:,:,0]
                    self.train_image1[1:10,:,:,1] = self.train_image[:,1:10,:,:,0] - self.train_image[:,0:9,:,:,0]
                    self.train_image1[5,:,:,:] = np.zeros((1,256,256,2))
                    self.train_image1[6,:,:,:] = np.zeros((1,256,256,2))
                    self.train_image1[7,:,:,:] = np.zeros((1,256,256,2))
                    self.train_image1[8,:,:,:] = np.zeros((1,256,256,2))
                    self.train_image1[9,:,:,:] = np.zeros((1,256,256,2))
                    self.train_image = np.asarray(self.train_image1).reshape((-1,self.num_flame,256,256,2))
...
def CNN(cell1, input_state1, image, keep_prob):
    with tf.name_scope('enc-conv1') as scope:
        image = tf.reshape(image,[-1]+[256,256]+[2])
    ...
    h_pool2_ = tf.reshape(h_pool2,[-1,tf.shape(image)[0]]+[64,64]+[32])
    ...
    with tf.name_scope('dec-conv1') as scope:
        ...
        h_conv3 = tf.nn.relu(deconv2d(outputs_, W_conv3,128,16,tf.shape(image)[0]) + b_conv3)

    with tf.name_scope('dec-conv2') as scope:
        W_conv4 = weight_variable([3, 3, 1, 16]) 
        b_conv4 = bias_variable([1])
        h_conv4 = tf.nn.tanh(deconv2d(h_conv3, W_conv4,256,1,tf.shape(image)[0]) + b_conv4)

    print image[:,:,:,0].shape
    ok = h_conv4 + tf.reshape(image[:,:,:,0],[tf.shape(image)[0]]+[256,256]+[1])
    ...
    with tf.name_scope('class-conv2') as scope:
        ...
        h_conv1_2 = tf.sigmoid(deconv2d(h_pool1_1, W_conv1_2,256,1,tf.shape(image)[0]) + b_conv1_2) 
...
if __name__ == '__main__':
    ...
    image1 = tf.placeholder(tf.float32, [None,None] + [256, 256]+ [2])
    y_r = tf.placeholder(tf.float32, [None,None] + [256, 256]+ [1])
    ...
    y_conv,output_state1,diff = CNN(cell1, input_state1, image1, keep_prob)
    loss2 = tf.sqrt(tf.reduce_mean(tf.square(y_conv - y_r)))
    #train_step = tf.train.AdamOptimizer(1e-2).minimize(loss)
    loss = tf.reduce_mean(tf.image.ssim(y_r[-1], y_conv[-1], 1.0))
    loss_gen = - tf.reduce_sum(y_r[-1] * tf.log( tf.clip_by_value(y_conv[-1],1e-20,1e+20)) + (1.-y_r[-1]) * tf.log( tf.clip_by_value(1.-y_conv[-1],1e-20,1e+20)))
    ...
    while(l <= 2000):
        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,y_conv], feed_dict={
                              image1: train_image[:,:5,:,:,:], y_r: train_r[:,1:6,:,:,:], keep_prob: 0.5})
                loss_a += loss_tmp[1]
                loss_b += loss_tmp[2]
                loss_c += loss_tmp[3]
                mid_state1 = loss_tmp[4]
                output_image = (loss_tmp[5] > 0.5) * 1.0
                train_image[:,5,:,:,0] = output_image[4,:,:,0]
                train_image[:,5,:,:,1] = output_image[4,:,:,0] - train_image[:,4,:,:,0]
                for m in range(4):
                   loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,y_conv], feed_dict={
                              image1: train_image[:,:m+6,:,:,:], y_r: train_r[:,1:m+7,:,:,:], keep_prob: 0.5})
                   if(True):
                      loss_a += loss_tmp[1]
                      loss_b += loss_tmp[2]
                      loss_c += loss_tmp[3]
                   mid_state1 = loss_tmp[4]
                   output_image = (loss_tmp[5] > 0.5) * 1.0
                   train_image[:,m+6,:,:,0] = output_image[m+5,:,:,0]
                   train_image[:,m+6,:,:,1] = output_image[m+5,:,:,0] - train_image[:,m+5,:,:,0]
                if(i % 1000 == 0 and i != 0):
                    print("train_step: " + str(i) +", ssim: "+str(loss_a/50)+", loss: "+str(loss_b/5000)+", loss_gen: "+str(loss_c/5000))
                    loss_a = 0
                    loss_b = 0
                    loss_c = 0
        if((l + 1) % 1 == 0):
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    loss_, states_1,output_image = sess.run([loss,output_state1,y_conv], feed_dict={
                                 image1: test_image[:,:5,:,:,:], y_r: test_r[:,1:6,:,:,:], keep_prob: 1.0})

                    rmse = rmse + loss_
                    state_test1 = states_1
                    test_image[:,5,:,:,0] = output_image[4,:,:,0]
                    test_image[:,5,:,:,1] = output_image[4,:,:,0] - test_image[:,4,:,:,0]
                    for m in range(4):
                        loss_, states_1,output_image = sess.run([loss,output_state1,y_conv], feed_dict={
                                 image1: test_image[:,:m+6,:,:,:], y_r: test_r[:,1:m+7,:,:,:], keep_prob: 1.0})


                        if(True):
                           rmse += loss_
                        state_test1 = states_1
                        test_image[:,m+6,:,:,0] = output_image[m+5,:,:,0]
                        test_image[:,m+6,:,:,1] = output_image[m+5,:,:,0] - test_image[:,m+5,:,:,0]
                        
                    num += 1
            rmse = rmse / (num*5)

            f = open("loss/model_m8_3.txt", "a")

            if(True):
                basermse = rmse
                save_path = saver.save(sess, "model/m8_3_"+str(l)+".ckpt")
