...
def make_datalist(filenames):
    ...
    for filename in filenames:
        image_file = glob.glob("/media/brainiv/PioMeidai/CIA_dataset/" +
                               filename + "/images/*")
        image_file.sort()
        train_image_label += image_file[:len(image_file)-100]
...
def CNN(cell1, input_state1, cell2, input_state2, cell3, input_state3, cell4, input_state4, cell5, input_state5, image, keep_prob):
    ...
    with tf.name_scope('dec-conv2') as scope:
        ...
        h_conv4 = tf.tanh(deconv2d(h_conv3, W_conv4,256,1,1) + b_conv4)
...
if __name__ == '__main__':
    ...
    total_loss = loss_pix_b #tf.reduce_sum(loss_pix_mask) - tf.reduce_sum(loss) + tf.reduce_sum(loss_norm)
        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,output_state2,output_state3,output_state4,output_state5,y_conv], feed_dict={
                              image1: [train_image[:,0,:,:,:]], y_r: [train_r[:,0,:,:,:]],y_mask: [im2ogm(train_r[:,0,:,:,:])], keep_prob: 0.5})
                loss_a += loss_tmp[1]
                loss_b += loss_tmp[2]
                loss_c += loss_tmp[3]
                ...
                for m in range(9):
                   if(m <= 3):
                       loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,output_state2,output_state3,output_state4,output_state5,y_conv], feed_dict={
                              image1: [train_image[:,m+1,:,:,:]],input_state1:mid_state1,input_state2:mid_state2,input_state3:mid_state3,input_state4:mid_state4,input_state5:mid_state5, y_r: [train_r[:,m+1,:,:,:]],y_mask: [im2ogm(train_r[:,m+1,:,:,:])], keep_prob: 0.5})
                       loss_a += loss_tmp[1]
                       loss_b += loss_tmp[2]
                       loss_c += loss_tmp[3]
                       mid_state1 = loss_tmp[4]
                       ...
                       loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,output_state2,output_state3,output_state4,output_state5,y_conv], feed_dict={
                              image1: output_image,input_state1:mid_state1,input_state2:mid_state2,input_state3:mid_state3,input_state4:mid_state4,input_state5:mid_state5, y_r: [train_r[:,m+1,:,:,:]],y_mask: [im2ogm(train_r[:,m+1,:,:,:])], keep_prob: 0.5})
                       loss_a += loss_tmp[1]
                       loss_b += loss_tmp[2]
                       loss_c += loss_tmp[3]
                       mid_state1 = loss_tmp[4]
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    loss_, states_1, states_2, states_3, states_4, states_5 = sess.run([loss_gen,output_state1,output_state2,output_state3,output_state4,output_state5], feed_dict={
                                 image1: [test_image[:,0,:,:,:]], y_r: [test_r[:,0,:,:,:]],y_mask: [im2ogm(train_r[:,0,:,:,:])], keep_prob: 1.0})
                    ...
                    for m in range(9):
                        if(m <= 3):
                           loss_, states_1, states_2, states_3, states_4,states_5,output_image = sess.run([loss,output_state1,output_state2,output_state3,output_state4,output_state5,y_conv], feed_dict={
                                 image1: [test_image[:,m+1,:,:,:]], input_state1:state_test1,input_state2:state_test2,input_state3:state_test3,input_state4:state_test4,input_state5:state_test5, y_r: [test_r[:,m+1,:,:,:]],y_mask: [im2ogm(train_r[:,m+1,:,:,:])], keep_prob: 1.0})
                        else:
                           loss_, states_1, states_2, states_3, states_4, states_5,output_image = sess.run([loss,output_state1,output_state2,output_state3,output_state4,output_state5,y_conv], feed_dict={
                                 image1: output_image, input_state1:state_test1,input_state2:state_test2,input_state3:state_test3,input_state4:state_test4,input_state5:state_test5, y_r: [test_r[:,m+1,:,:,:]],y_mask: [im2ogm(train_r[:,m+1,:,:,:])], keep_prob: 1.0})
