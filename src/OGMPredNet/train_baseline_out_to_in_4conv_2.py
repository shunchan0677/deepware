        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                for m in range(8):
                   if(True):
                       loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,output_state2,output_state3,output_state4,y_conv], feed_dict={
                              image1: [train_image[:,m+1,:,:,:]],input_state1:mid_state1,input_state2:mid_state2,input_state3:mid_state3,input_state4:mid_state4, y_r: [train_r[:,m+2,:,:,:]], keep_prob: 0.5})
                   else:
                       loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,output_state2,output_state3,output_state4,y_conv], feed_dict={
                              image1: [output_image],input_state1:mid_state1,input_state2:mid_state2,input_state3:mid_state3,input_state4:mid_state4, y_r: [train_r[:,m+2,:,:,:]], keep_prob: 0.5})
                   loss_a += loss_tmp[1]
                   loss_b += loss_tmp[2]
                   loss_c += loss_tmp[3]
                   mid_state1 = loss_tmp[4]
        ...
        if((l + 1) % 1 == 0):
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    loss_, states_1, states_2, states_3, states_4 = sess.run([loss,output_state1,output_state2,output_state3,output_state4], feed_dict={
                    ...
                    for m in range(8):
                        if(m <= 3):
                           ...
                           loss_, states_1, states_2, states_3, states_4,output_image = sess.run([loss,output_state1,output_state2,output_state3,output_state4,y_conv], feed_dict={
                                 image1: [(output_image > 0.5) * 1.0], input_state1:state_test1,input_state2:state_test2,input_state3:state_test3,input_state4:state_test4, y_r: [test_r[:,m+2,:,:,:]], keep_prob: 1.0})
            ...
            f = open("loss/model_m3_3.txt", "a")

            if(True):
                basermse = rmse
                save_path = saver.save(sess, "model/m3_3_"+str(l)+".ckpt")
