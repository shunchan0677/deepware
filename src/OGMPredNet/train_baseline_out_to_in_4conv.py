...
def CNN(cell1, input_state1, cell2, input_state2, cell3, input_state3, cell4, input_state4, image, keep_prob):
    ...
    return h_conv1_2, state1,state2,state3,state4, h_conv4
    ...
    while(l <= 2000):
        ...
        for i, train_data in enumerate(train_new_generator):
            if not train_data:
                ...
                loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,output_state2,output_state3,output_state4,y_conv], feed_dict={
                ...
                output_image = (loss_tmp[8] > 0.5) * 1.0
                for m in range(9):
                   if(m <= 3):
                       loss_tmp = sess.run([loss,loss2,loss_gen,output_state1,output_state2,output_state3,output_state4,y_conv], feed_dict={
                              image1: [train_image[:,m+1,:,:,:]],input_state1:mid_state1,input_state2:mid_state2,input_state3:mid_state3,input_state4:mid_state4, y_r: [train_r[:,m+1,:,:,:]], keep_prob: 0.5})
                       loss_a += loss_tmp[0]
                       loss_b += loss_tmp[1]
                       loss_c += loss_tmp[2]
                       mid_state1 = loss_tmp[3]
                       mid_state2 = loss_tmp[4]
                       mid_state3 = loss_tmp[5]
                       mid_state4 = loss_tmp[6]
                       output_image = (loss_tmp[7] > 0.5) * 1.0
                   else:
                       loss_tmp = sess.run([train_step,loss,loss2,loss_gen,output_state1,output_state2,output_state3,output_state4,y_conv], feed_dict={
                              image1: [output_image],input_state1:mid_state1,input_state2:mid_state2,input_state3:mid_state3,input_state4:mid_state4, y_r: [train_r[:,m+1,:,:,:]], keep_prob: 0.5})
                       loss_a += loss_tmp[1]
                       loss_b += loss_tmp[2]
                       loss_c += loss_tmp[3]
                       mid_state1 = loss_tmp[4]
                       ...
                       output_image = (loss_tmp[8] > 0.5) * 1.0
                if(i % 1000 == 0 and i != 0):
                    print("train_step: " + str(i) +", ssim: "+str(loss_a/100)+", loss: "+str(loss_b/10000)+", loss_gen: "+str(loss_c/10000))
                    loss_a = 0
                    loss_b = 0
                    loss_c = 0
        if((l + 1) % 1 == 0):
            ...
            for i, test_data in enumerate(test_new_generator):
                if not test_data:
                    ...
                    loss_, states_1, states_2, states_3, states_4 = sess.run([loss_gen,output_state1,output_state2,output_state3,output_state4], feed_dict={
                    ...
                    for m in range(9):
                        if(m <= 3):
                           loss_, states_1, states_2, states_3, states_4,output_image = sess.run([loss,output_state1,output_state2,output_state3,output_state4,y_conv], feed_dict={
                                 image1: [test_image[:,m+1,:,:,:]], input_state1:state_test1,input_state2:state_test2,input_state3:state_test3,input_state4:state_test4, y_r: [test_r[:,m+1,:,:,:]], keep_prob: 1.0})
                        else:
                           loss_, states_1, states_2, states_3, states_4,output_image = sess.run([loss,output_state1,output_state2,output_state3,output_state4,y_conv], feed_dict={
                                 image1: [(output_image > 0.5) * 1.0], input_state1:state_test1,input_state2:state_test2,input_state3:state_test3,input_state4:state_test4, y_r: [test_r[:,m+1,:,:,:]], keep_prob: 1.0})
            ...
            f = open("loss/model_m5_3.txt", "a")

            if(True):
                basermse = rmse
                save_path = saver.save(sess, "model/m5_3_"+str(l)+".ckpt")
